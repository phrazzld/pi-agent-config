...
3. P0 done: Thin workflow parity via prompt templates (`execute`, `spec`, `architect`, `pr`, `respond`, `fix-ci`, `groom`, `autopilot`).
4. P1 done: Profile modes extension (`ultrathink`, `execute`, `ship`, `fast`).
5. P1 pending: Add optional multi-agent orchestrator command once stable baseline usage is measured.
6. P1 pending: Add extension-level metrics rollup (counts, latency, cache hit rate) for weekly review.
